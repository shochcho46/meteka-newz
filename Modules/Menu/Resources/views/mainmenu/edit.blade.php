...
@include('common.message')
    <div class="row">
        ...
        <div class="col-xxl-4 col-xl-4 col-md-6 col-sm-10">
            ...
            <form action="{{ route('mainmenu.update',$data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <div class="card-style mb-30">
                    <h4 class="mb-25">{{ __('webstring.edit') }} {{ __('webstring.main_menu') }}</h4>

                    <div class="col-12">
                        <div class="input-style-2">
                        <label for="name" class="form-label">{{ __('webstring.main_menu') }}</label>
                        <input type="text" class="form-control" id="name" name="name" placeholder="{{ __('webstring.main_menu') }}"  value="{{ old('name') ?? $data->name  }}" >
