        ...
        <div class="col-xxl-4 col-xl-4 col-md-6 col-sm-10">
            ...
            <form action="{{ route('submenu.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-style mb-30">
                    <h4 class="mb-25">{{ __('webstring.add') }} {{ __('webstring.sub_menu') }}</h4>
                             ...
                             <div class="select-position">
                               <select  name="mainmenu_id" id="mainmenu_id"  required>
                                @foreach ($mainmenu as $mainValue)
                                    <option value="{{ $mainValue->id }}"> {{ $mainValue->name }}</option>
                                @endforeach
                                </select>
                             </div>
                         </div>
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="name" name="name" placeholder="{{ __('webstring.sub_menu') }}"  value="{{ old('name') }}" >
